const mySecret = process.env['vaikan@tmails.co.uk']
...
App.get('/', async(Req, Res) => Res.send('THE DEVELOPERS :: N7R'))
...
client.on('messageCreate', async Message => {
    ...
    if (Command == 'setup') {
        ...
        const Embed = new EmbedBuilder()
            ...
            .setDescription(`Voice control panel`)
            .setTimestamp()
        ...
        const RowThree = new ActionRowBuilder()
            .addComponents(
                ...
                new ButtonBuilder()
                    ...
                    .setLabel('Delete Temporary Channel')
...
client.on('voiceStateUpdate', async (OldVoice, NewVoice) => {
    if (NewVoice.channelId == BotConfig.ChannelId) {
        await NewVoice.guild.channels.create({
            name: `(🔊) voice : ${NewVoice.member.user.username}`,
...
client.login(process.env.ClientToken).catch(() => {
